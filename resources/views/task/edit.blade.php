...
jQuery(document).delegate('button.btn-add', 'click', function(e) {
    ...
    if(status == 'gagal'){
        ...
        //append tr
        var markup = '<tr id="rec-'+ size +'"><td class="text-center"><span class="sn">'+ size +'</span></td><td>'+ task +'<input type="hidden" name="task[]" value="'+ task +'"></td><td>'+ jam_mulai +'<input type="hidden" name="jam_mulai[]" value="'+ jam_mulai +'"></td><td>'+ jam_selesai +'<input type="hidden" name="jam_selesai[]" value="'+ jam_selesai +'"><input type="hidden" name="id_task[]" value="0"></td><td>'+ project +'<input type="hidden" name="project[]" value="'+ id_project +'"></td><td class="text-center"><button class="btn btn-icon btn-sm btn-danger btn-delete" data-id="'+ size +'"><i class="fa fa-trash"></i></button></td></tr>';
            ...
            <div class="block-content">
                <div class="form-group row">
                    <div class="col-lg-3 col-sm-12">
                        ...
                        <input type="text" name="tanggal" id="tanggal" class="js-flatpickr form-control bg-white" placeholder="d-m-Y" data-date-format="d-m-Y" value="{{ tgl_format($tanggal) }}" disabled="">
                    </div>
                </div>
                <div class="form-row">
                    ...
                    <div class="form-group col-lg-2">
                        ...
                        <select name="projects" id="project" class="js-select2 form-control">
                            ...
                            @foreach ($project as $key => $datas)
                            <option value="{{ $key }}">{{ $datas }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-lg-2">
                        <button class="btn btn-icon btn-sm btn-primary btn-add" id="btn-add"><i class="fa fa-plus"></i>&nbsp;Tambah Task</button>
                    </div>
                </div>
                ...
                <table class="table table-bordered w-100" id="table-task">
                    ...
                    <tbody id="tbody_task">
                        ...
                        <tr id="rec-{{ $no }}">
                            ...
                            <td>
                                <input type="text" name="task[]" class="form-control" value="{{ $datas->task }}">
                            </td>
                            <td>
                                <input type="text" name="jam_mulai[]" class="js-flatpickr form-control bg-white" value="{{ $datas->jam_mulai }}" data-enable-time="true" data-no-calendar="true" data-date-format="H:i" data-time_24hr="true">
                            </td>
                            <td>
                                <input type="text" name="jam_selesai[]" class="js-flatpickr form-control bg-white" value="{{ $datas->jam_selesai }}" data-enable-time="true" data-no-calendar="true" data-date-format="H:i" data-time_24hr="true">
                                <input type="hidden" name="id_task[]" value="{{ $datas->id }}">
                            </td>
                            <td>
                                <select name="project[]" class="js-select2 form-control">
                                    <option value="" {{ $datas->project_id == "" ? "selected" : "" }} selected disabled>Pilih Project</option>
                                    @foreach ($project as $key => $data)
                                    <option value="{{ $key }}" {{ $datas->project_id == $key ? "selected" : "" }}>{{ $data }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td class="text-center">
                                <button class="btn btn-icon btn-sm btn-danger btn-delete" data-id="{{ $no }}"><i class="fa fa-trash"></i></button>
                            </td>
                        </tr>
                        @php
                            $no++;
